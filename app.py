...
def parse_intent(user_text: str) -> IntentParse:
    sys = SystemMessage(content=(
        ...
        "- ABOUT_FETII (for 'What is Fetii?', booking, pricing, vehicles, support)\n"
...
SIZE_RANGE = re.compile(r"\b(\d+)\s*(?:to|-|–|—)\s*(\d+)\b")           # e.g., 3-5
SIZE_AT_LEAST = re.compile(r"\b(at least|>=|≥)\s*(\d+)\b", re.I)
SIZE_N_PLUS = re.compile(r"\b(\d+)\s*\+\b", re.I)                      # 6+
...
def faq_lookup(user_text: str) -> Optional[str]:
    ...
    if any(k in t for k in ["how do i book", "how to book", "book in app", "booking"]):
...
def handle_top_dropoffs_by_age_and_day(age_low: Optional[int],
    ...
    # fallback: most recent month with any matches
...
def handle_about_fetii(_: str) -> str:
    return (
        "Fetii is a group rideshare platform that helps move **groups** together efficiently—"
        "popular for nights out, events, and corporate transport.\n\n"
        "**How to book (in the Fetii mobile app):**\n"
        "1) Open the Fetii app (iOS/Android) and sign in or create an account.\n"
        "2) Enter your pickup & drop-off, group size, and preferred time.\n"
        "3) Choose a vehicle option that fits your group.\n"
        "4) Confirm the ride in the app; you’ll get driver details and status updates.\n\n"
        "**Why use Fetii?** Move together with fewer split rides, easier coordination, "
        "often cheaper per person, and more fun.\n\n"
        "**Support:** use the in-app Help to reach Fetii support. (In this demo, the ‘Contact Fetii/Support’ "
        "button in the sidebar is a placeholder.)\n\n"
        "_Want to explore data too? Try: 'How many groups went to Moody Center last month?'_"
    )
...
def process(state: AgentState) -> AgentState:
    ...
    if parsed.intent == "COUNT_TRIPS_TO_LOCATION":
        ...
        response = handle_about_fetii(user_text)
    elif parsed.intent == "HELP_FAQ":
        response = handle_help_faq()
    else:
        # Improved fallback for generic questions
        low = user_text.lower()
        faq = faq_lookup(user_text)
        if faq:
            response = faq + "\n\n(You can also ask data questions like “How many groups went to Moody Center last month?”)"
        ...
        elif any(k in low for k in ["what is fetii", "how do i book", "book in app", "pricing", "vehicles", "support", "why use"]):
            response = handle_about_fetii(user_text)
        else:
...
# -------------------------------
# Bottom suggestion panel + chat_input
# -------------------------------
# If there is a pending draft from the sidebar, show it right above the bottom input
...
# `st.chat_input` stays pinned at the bottom of the page and scrolls with chat
